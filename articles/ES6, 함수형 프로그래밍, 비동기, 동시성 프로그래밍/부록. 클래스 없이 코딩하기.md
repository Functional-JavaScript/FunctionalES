...
```javascript
const calls = (fs, a) => map(f => f(a), fs);

const add = (a, b) => a + b;
const sub = (a, b) => a - b;
const mult = (a, b) => a * b;

const beforeCalls = (a, fs, calc) => go(
  calls(fs, a),
  reduce(calc));

beforeCalls(
  0,
  [a => a + 10, a => a + 20], // 0 + 10, 0 + 20
  add);
  // 30

beforeCalls(
  5,
  [a => a + 10, a => a + 20], // 5 + 10, 5 + 20
  add);
  // 40

// 화살표 함수를 이용하여 sub, mult를 부분 적용하고,
// fs와, a를 전달 받는 순서를 바꾸고,
// (...fs) 한 번, a 한 번 나누어 적용되도록 만들어진 함수를 리턴
const subF = (...fs) => a => beforeCalls(a, fs, sub);
const multF = (...fs) => a => beforeCalls(a, fs, mult);
